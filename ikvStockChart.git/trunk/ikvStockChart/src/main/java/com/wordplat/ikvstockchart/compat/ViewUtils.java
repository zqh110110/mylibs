...
public class ViewUtils {
     ...
     * 设置 蜡烛图画笔的颜色和是否空心
     */
    public static Entry setUpTimePaint(Paint candlePaint, EntrySet entrySet, int currentEntryIndex, SizeColor sizeColor) {
        Entry preEntry = null;
        if (currentEntryIndex>0) {
            preEntry = entrySet.getEntryList().get(currentEntryIndex - 1);
        }
        Entry entry = entrySet.getEntryList().get(currentEntryIndex);
        ...
        else if (entry.getClose() > preEntry.getClose()) {
            candlePaint.setColor(sizeColor.getIncreasingColor());
        } else if (entry.getClose() == preEntry.getClose()) {
            candlePaint.setColor(sizeColor.getNeutralColor());
        } else {
