...
import com.kfd.activityfour.TranslucentActivity;
...
public class MyAnimations {
	...
	public static void startAnimationsIn(ViewGroup viewgroup, int durationMillis) {
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			...
			Animation animation = new TranslateAnimation(mlp.rightMargin - xOffset- 60/(i+1), 0F, yOffset + mlp.bottomMargin+110, 0F);
	...
	// 图标的动画(出动画)
	public static void startAnimationsOut(final Context context ,final RelativeLayout viewgroup, int durationMillis) {
	
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			final ImageView inoutimagebutton = (ImageView) viewgroup.getChildAt(i);
			MarginLayoutParams mlp = (MarginLayoutParams) inoutimagebutton.getLayoutParams();
			Animation animation = new TranslateAnimation(0F, mlp.rightMargin - xOffset- 60/(i+1),0F, yOffset + mlp.bottomMargin+120 );

			animation.setFillAfter(true);
			animation.setDuration(durationMillis);
			animation.setStartOffset(((viewgroup.getChildCount() + i) * 20) / (-1 + viewgroup.getChildCount()));// 下一个动画的偏移时间
			animation.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation arg0) {
					java.util.Timer timer = new java.util.Timer(true);
					 TimerTask task = new TimerTask() {
						    public void run() {  
						    	((TranslucentActivity) context).finshActivity();
						    }
					    };   
