...
public class TimeButton extends Button implements OnClickListener {
	...
	public void onDestroy() {
		if (AppContext.timermap == null)
			AppContext.timermap = new HashMap<String, Long>();
		AppContext.timermap.put(TIME, time);
		AppContext.timermap.put(CTIME, System.currentTimeMillis());
		clearTimer();
	...
	public void onCreate(Bundle bundle) {
		Log.e("yung", AppContext.timermap + "");
		if (AppContext.timermap == null)
			return;
		if (AppContext.timermap.size() <= 0)// 这里表示没有上次未完成的计时
			return;
		long time = System.currentTimeMillis() - AppContext.timermap.get(CTIME)
				- AppContext.timermap.get(TIME);
		AppContext.timermap.clear();
		if (time > 0)
			return;
		else {
			initTimer();
	 ...
	 * @return
	 */
	public TimeButton setLength(long lenght) {
