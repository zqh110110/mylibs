	...
	public static FuturesPositionListBean parseData(String string){
		...
		try {
			if (string != null && string.length() > 0) {
				...
				JSONObject jsonObject1 = jsonObject.getJSONObject("data");
				JSONArray array = jsonObject1.getJSONArray("liveList");

				for (int i = 0; i < array.size(); i++) {
					JSONObject jsonObject2 = array.getJSONObject(i);
					FuturesPositionBean positionBean = new FuturesPositionBean();
					positionBean.setPid(jsonObject2.getString("pid"));
					...
					positionBean.setNow_price(jsonObject2
							.getString("now_price"));
					...
					//positionBean.setFutulastp(jsonObject2.getString("futulastp"));
					positionBean.setFloatMoney(jsonObject2.getString("floatMoney"));
