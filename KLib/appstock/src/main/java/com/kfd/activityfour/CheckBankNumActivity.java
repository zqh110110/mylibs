 ...
 */
public class CheckBankNumActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.checkbankid);
		if (getIntent().getSerializableExtra("bean") != null) {
			customInfoBean = (CustomInfoBean) getIntent().getSerializableExtra("bean");
		}
		initTitle();
		initUI();
	...
	private EditText oldpasswordEditText, cardidEditText,
		banknameEditText,bankidEditText,zhihangEditText;
	
	private LinearLayout llLayout1 ,llLayout2,llLayout3,llLayout4,llLayout5;
	...
	private void initUI() {
		...
		llLayout4 = (LinearLayout) findViewById(R.id.lin4);
		llLayout5 = (LinearLayout) findViewById(R.id.lin5);
		...
		cardidEditText = (EditText) findViewById(R.id.editText2);
		
		if (customInfoBean.getCard_no()!=null) {
			if (customInfoBean.getCard_no().toString().equals("")) {
				
			} else {
				cardidEditText.setText(customInfoBean.getCard_no());
				cardidEditText.setEnabled(false);
			}		
		}
		
		banknameEditText = (EditText) findViewById(R.id.editText3);
		bankidEditText = (EditText) findViewById(R.id.editText4);
		zhihangEditText = (EditText) findViewById(R.id.editText5);
		
		if (customInfoBean.getAddress()!=null) {
			if (customInfoBean.getAddress().toString().equals("")) {
				
			} else {
				zhihangEditText.setText(customInfoBean.getAddress());
				zhihangEditText.setEnabled(false);
			}		
		}
		
		if (customInfoBean.getMaccount()!=null) {
			if (customInfoBean.getMaccount().toString().equals("")) {
				
			} else {
				bankidEditText.setText(customInfoBean.getMaccount());
				bankidEditText.setEnabled(false);
			}		
		}
		
		if (customInfoBean.getBark_name()!=null) {
			if (customInfoBean.getBark_name().toString().equals("")) {
				
			} else {
				banknameEditText.setText(customInfoBean.getBark_name());
		...
		commitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String cardid = cardidEditText.getText().toString()
						.trim();
				String bankname = banknameEditText.getText()
						.toString().trim();
				String bankid = bankidEditText.getText()
						.toString().trim();
				String zhihang = zhihangEditText.getText()
						.toString().trim();
				
				checkbank(oldpassword, cardid, bankname,bankid,zhihang);
			}
		});
	...
	private void checkbank(final String oldpassword,
			final String cardid, final String bankname, final String bankid, final String zhihang) {
		try {
			progressDialog = new ProgressDialog(getParent());
			progressDialog.setMessage("正在验证银行账号，请稍候.....");
		...
		executorService.execute(new Runnable() {
			...
			public void run() {
					...
					if (customInfoBean.getBank_has()!=null) {
						if (customInfoBean.getBank_has().toString().equals("-1")) {
							hashMap.put("request", "accountEditbank");
						} else {
							hashMap.put("request", "accountSetbank");
						}					
					}else {
						hashMap.put("request", "accountSetbank");
					}
					...
					hashMap.put("bark_name", bankname);
					hashMap.put("address", zhihang);
					hashMap.put("maccount", bankid);
					if (customInfoBean.getCard_no()!=null) {
						if (customInfoBean.getCard_no().toString().equals("")) {
							hashMap.put("card_no", cardid);
						} else {
	...
	private Handler updateUIHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 2:
				...
				llLayout5.setVisibility(View.VISIBLE);
